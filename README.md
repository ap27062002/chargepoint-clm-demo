# ChargePoint Enterprise CLM — Guided Demo (forked from chargepoint-clm)

This is a **fork of the full chargepoint-clm app** with one addition: a dismissible **Demo Guide**
overlay (`src/components/DemoGuide.tsx`) that walks a two-persona, ticket-to-signature story —
sales rep (Marcus, initiator) creates a deal, attorney (Kirsten) queues it, analyzes redlines
across all three document types on the Northwind Energy multi-agreement ticket (MSA/DPA/SOW),
sends back, receives the counterparty's response, and proceeds to execution/signing.

Nothing about the underlying app was removed or simplified — every functionality from the main
build is still here and reachable normally. The guide just drives the real app to the right
screen/persona at each step via real store actions (not a scripted replay), and can be hidden at
any time to freely explore or explain other features live, then resumed via the "Resume demo
guide" pill.

Also added: real document content (clause bodies + tracked changes) for the Northwind DPA and SOW
in `src/data/documents.ts` — previously only the MSA had a populated document; now all three of
the multi-agreement ticket's document types have something real to show.
